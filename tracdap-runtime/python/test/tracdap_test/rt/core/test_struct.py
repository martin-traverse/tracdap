...
class StructProcessingTest(unittest.TestCase):
    ...
    def define_struct_schema(sub_struct_type: type):
        ...
        return _meta.StructSchema(
            fields={

                "boolField": _meta.FieldSchema(
                ...
                "decimalField": _meta.FieldSchema(
                ...
                "datetimeField": _meta.FieldSchema(
                ...
                "structField": _meta.FieldSchema(
                    fieldName="structField",
                    fieldOrder=13,
                    fieldType=_meta.BasicType.STRUCT,
                    label="",
                    notNull=False,
                    typeName=f"{sub_struct_type.__module__}.{sub_struct_type.__name__}"),

                "nestedStructField": _meta.FieldSchema(
                ...
                "nestedOptionalStructField": _meta.FieldSchema(
            ...
            namedTypes={
                f"{sub_struct_type.__module__}.{sub_struct_type.__name__}": _meta.StructSchema(
                    fields={
                        "field1": _meta.FieldSchema(
                            fieldName="field1", fieldOrder=0, fieldType=_meta.BasicType.STRING, notNull=True,
                            label="", defaultValue=None),

                        "field2": _meta.FieldSchema(
    ...
    def _assert_schema_equal(self, expected: _meta.StructSchema, actual: _meta.StructSchema):

        self.assertSetEqual(set(expected.fields.keys()), set(actual.fields.keys()))

        for key, expected_field in expected.fields.items():

            actual_field = actual.fields.get(key)
