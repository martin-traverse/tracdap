...
class LoadDataFunc( _LoadSaveDataFunc, NodeFunction[_data.DataItem],):
    ...
    def _load_table(self, data_spec, data_copy):
        ...
        trac_schema = data_spec.schema_def if data_spec.schema_def else data_spec.primary_def.schema
...
class RunModelFunc(NodeFunction[Bundle[_data.DataView]]):
    ...
    def _execute(self, ctx: NodeContext) -> Bundle[_data.DataView]:
        ...
        if any(dynamic_outputs):
            update = _graph.GraphBuilder.build_dynamic_outputs(self.node.id, dynamic_outputs)
...
class JobResultFunc(NodeFunction[_cfg.JobResult]):
    ...
    def _execute(self, ctx: NodeContext) -> _cfg.JobResult:
        ...
        # if self.node.dynamic_outputs is not None:
        #     for result_node in self.node.dynamic_outputs:
        #     runtime_outputs = _ctx_lookup(self.node.runtime_outputs, ctx)
        #     self._process_outputs(runtime_outputs, ctx, job_result, result_def)
    ...
    def _process_data_spec(output_name: tp.Optional[str], data_spec: _data.DataSpec, job_result: _cfg.JobResult):
        ...
        output_def = data_spec.primary_def
        ...
        storage_def = data_spec.storage_def
