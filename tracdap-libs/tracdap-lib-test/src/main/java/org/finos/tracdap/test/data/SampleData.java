...
public class SampleData {
    ...
    public static ArrowContext generateBasicData(BufferAllocator arrowAllocator) {
    ...
    public static ArrowContext convertData(
    ...
    public static ArrowContext convertData(
            ArrowSchema schema, Map<String, List<Object>> data,
            int size, BufferAllocator arrowAllocator) {

        for (var fieldName : data.keySet()) {
            if (schema.primarySchema().findField(fieldName) == null) {
                throw new ETracInternal("Sample data field " + fieldName + " is not present in the schema");
            }
        }

        var context = ArrowContext.forSchema(schema, arrowAllocator);

        FieldVector vector;
        BiConsumer<Integer, Object> setFunc;

        for (int col = 0, nCols = schema.concreteSchema().getFields().size(); col < nCols; col++) {

            var field = schema.concreteSchema().getFields().get(col);
