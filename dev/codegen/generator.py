...
class TracGenerator:
    ...
    SERVICE_CLASS_TEMPLATE = (
        '{INDENT}class {SERVICE_NAME}(_abc.ABC):\n'
        '\n'
        '{DOC_COMMENT}'
        '{METHODS}')

    SERVICE_METHOD_TEMPLATE = (
        '{INDENT}@_abc.abstractmethod\n'
        '{INDENT}def {METHOD_NAME}(self, request: {REQUEST_TYPE}) -> {RESPONSE_TYPE}:  # noqa\n'
        '\n'
        '{DOC_COMMENT}'
        '{NEXT_INDENT}pass\n\n')
    ...
    def generate_package_imports(self, descriptor: pb_desc.FileDescriptorProto) -> str:
        ...
        for service_type in descriptor.service:
            imports += self.PACKAGE_IMPORT_TEMPLATE.format(
                MODULE_NAME=module_name,
                SYMBOL=service_type.name)
    ...
    def generate_file(
        ...
        code = self.FILE_TEMPLATE \
            ...
            .replace("{PKG_IMPORTS}", pkg_imports) \
            .replace("{ENUMS_CODE}", enums_code) \
            .replace("{MESSAGES_CODE}", messages_code) \
            .replace("{SERVICES_CODE}", services_code)
    ...
    def generate_service_method(
        ...
        request_type = self.python_type_name(package, descriptor.input_type, make_relative=True, alias=True)
        response_type = self.python_type_name(package, descriptor.output_type, make_relative=True, alias=True)
